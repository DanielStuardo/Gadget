...
#define    WEIGHT    2
    ...
    Iterator up i [0:1:edges] {
             //int vert_origen = $graph[i,VERT_ORIG]-1;
             //int vert_destino = $graph[i,VERT_DEST]-1;
             //$processWeights[ vert_origen, vert_destino ] = $graph[i,WEIGHT];
             // NO PUEDO HACER CELL ANIDADO?... DEBE SER POR EL PROCESO CONCAT2 :(
             //$processWeights[ $graph[i,VERT_ORIG]-1, $graph[i,VERT_DEST]-1 ] = $graph[i,WEIGHT];
             
             $2processWeights[ $graph[i,VERT_ORIG]-1, $graph[i,VERT_DEST]-1 ] = $graph[i,WEIGHT];
             // estas sí funcionan, pero se ve feo y largo:
             //$processWeights[ Cell2(graph,i,VERT_ORIG)-1, Cell2(graph,i,VERT_DEST)-1 ] = $graph[i,WEIGHT];
             //Cell2( processWeights, Cell2(graph,i,0)-1, Cell2(graph,i,1)-1 ) = $graph[i,WEIGHT];
    }
...
int * CargaGrafo( pRDS(int, graph), const char *cFile){
   ...
   if(dataFile.is_matrix){
       ...
       if( graph ){
           ...
           Block( vertices, /*pFix( col, graph, 0 ); Range ptr graph [0:1:pRows(graph)];*/
                            Range ptr graph [ 0:1:pRows(graph), 0:1:1 ];

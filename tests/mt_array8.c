    ...
    if(Exist_file("save_matrix.txt"))
    {
       /* recupero informacion del archivo para su apertura segura */
        ...
        if(dataFile.is_matrix)   // tiene forma de matriz???
        {
            ...
            /* preparo la columna a insertar */
            New_mt_array(nueva_col);
            sAppend_mt(nueva_col,"head X");
            Append_mt(nueva_col,float,0.0);
            Append_mt(nueva_col,int,0);
            Append_mt(nueva_col,double,0.0);
            Append_mt(nueva_col,long, 0L);
            Append_mt(nueva_col,long, 0L);
            uAppend_mt(nueva_col,long, 0L);
    
            /* insertamos la misma columna en el array, 3 veces */
            test = Insert_col_mt(pSDS(test),pSDS(nueva_col), 2);
            test = Insert_col_mt(pSDS(test),pSDS(nueva_col), 2);
            test = Insert_col_mt(pSDS(test),pSDS(nueva_col), 5);

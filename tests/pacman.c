...
Main
 ...
 while ( vidas > 0 ){
   ...
   /* datos de los phantoms */
    ...
    print_dots( 226,BACKGROUND );//, status_pills, sw_power_pills );
    //print_tunnel();
...
//             At x+5,y+20; draw_ascii(pacman,2,119,0,boca);
    ...
    if ( sw_level_clear ){
             kill_all_sounds();
             Free secure PIDEATGHOST, PIDWAKA, PIDPILLS,PIDSIREN;
             At x,y; draw_ascii_pacman(226,BACKGROUND,0,1);
             sleep(1);
             cnt_void_waka=0;
             sw_waka=0;
             sw_dead=0;
             ...
             Iterator up i[0:1:5]{
                 ...
                 //At 1,1; 
                 Color( (sw_color ? color_level[level] : 226), BACKGROUND); Print "%s",lab;
                 Flush_out;
                 usleep(550000);
...
void initial_screen()
{
   ...
   //  Enable_raw_mode();
     ...
     while ( !Kbhit() ){
         When ( Timer( &t, 80L ) ){
             At 22,36; draw_ascii_pacman(226,BACKGROUND,DIR_LEFT,boca);
             boca = boca ? 0 : 1;
         }
         When ( Timer( &tp, 220L ) ){
            if (sw_power_pills){
                  ...
                  int px[], int py[], int x, int y )
{
   ...
   pone_miniaturas(vidas-1, level_fruit);
   At x,y; draw_ascii_pacman(226,BACKGROUND,0,0); // 77,36
   Iterator up i[0:1:4]{
